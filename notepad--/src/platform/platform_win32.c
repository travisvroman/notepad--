...
static void platform_update_watches(platform_state *state);
...
b8 platform_system_startup(u64 *memory_requirement, struct platform_state *state, void *config) {
    ...
    *memory_requirement = sizeof(platform_state);
    ...
    /* wc.cbWndExtra = 8;  // Space to hold the platform_state pointer. */
    ...
    if (!RegisterClassA(&wc)) {
        ...
        if (dw) {
            KERROR("Window registration error: '%s'.", lpMsgBuf);
        }
    ...
    HWND handle = CreateWindowExA(
        ...
        0, 0, state->handle.h_instance, state);
    ...
    ShowWindow(state->handle.hwnd, show_window_command_flags);
...
struct platform_handle *platform_handle_get(struct platform_state *state) {
    if (!state) {
        KERROR("platform_handle_get requires a valid pointer to state. Null has been returned.");
        return 0;
    }
...
void platform_sleep(struct platform_state *state, u64 ms) {
    Sleep(ms);
}
...
void platform_get_handle_info(struct platform_state *state, u64 *out_size, void *memory) {
    *out_size = sizeof(platform_handle);
    if (!memory) {
        return;
    }

    kcopy_memory(memory, &state->handle, *out_size);
}

f32 platform_device_pixel_ratio(struct platform_state *state) {
    return state->device_pixel_ratio;
}
...
void kthread_sleep(kthread *thread, u64 ms) {
    platform_sleep(platform_get_state(), ms);
}
...
b8 platform_dynamic_library_load(struct platform_state *state, const char *name, dynamic_library *out_library) {
    if (!out_library) {
        return false;
    }
...
b8 platform_dynamic_library_unload(struct platform_state *state, dynamic_library *library) {
    if (!library) {
        return false;
    }
...
b8 platform_dynamic_library_load_function(struct platform_state *state, const char *name, dynamic_library *library) {
    if (!name || !library) {
        return false;
    }
...
const char *platform_dynamic_library_extension(struct platform_state *state) {
    return ".dll";
}

const char *platform_dynamic_library_prefix(struct platform_state *state) {
    return "";
}

platform_error_code platform_copy_file(struct platform_state *state, const char *source, const char *dest, b8 overwrite_if_exists) {
...
static b8 register_watch(struct platform_state *state, const char *file_path, u32 *out_watch_id) {
    if (!state || !file_path || !out_watch_id) {
        if (out_watch_id) {
    ...
    *out_watch_id = INVALID_ID;
    ...
    if (!state->watches) {
        state->watches = darray_create(win32_file_watch);
    }
    ...
    u32 count = darray_length(state->watches);
    for (u32 i = 0; i < count; ++i) {
        win32_file_watch *w = &state->watches[i];
        if (w->id == INVALID_ID) {
    ...
    w.last_write_time = data.ftLastWriteTime;
    *out_watch_id = count;
    darray_push(state->watches, w);

    return true;
}

static b8 unregister_watch(struct platform_state *state, u32 watch_id) {
    ...
    win32_file_watch *w = &state->watches[watch_id];
    w->id = INVALID_ID;
...
b8 platform_watch_file(struct platform_state *state, const char *file_path, u32 *out_watch_id) {
    return register_watch(state, file_path, out_watch_id);
}

b8 platform_unwatch_file(struct platform_state *state, u32 watch_id) {
    return unregister_watch(state, watch_id);
}

static void platform_update_watches(struct platform_state *state) {
    if (!state || !state->watches) {
        return;
    }
    ...
    u32 count = darray_length(state->watches);
    for (u32 i = 0; i < count; ++i) {
        win32_file_watch *f = &state->watches[i];
        if (f->id != INVALID_ID) {
            WIN32_FIND_DATAA data;
...
LRESULT CALLBACK win32_process_message(HWND hwnd, u32 msg, WPARAM w_param, LPARAM l_param) {
    platform_state *state = (platform_state *)GetWindowLongPtrA(hwnd, 0);
    switch (msg) {
        case WM_ERASEBKGND:
        ...
        case WM_DPICHANGED:
            ...
            // TODO: This should be per window when multi-window exists.
            state->device_pixel_ratio = (f32)x_dpi / USER_DEFAULT_SCREEN_DPI;
